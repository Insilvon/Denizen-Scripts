...
DuelFormat:
    type: format
    format: "<&8>[Duel]<&co><&7> <text>"

DuelActionFormat:
    type: format
    format: "<&8><&gt><&gt><&co><&7> <text>"
        ...
        - choose <[command]>:
            ...
            - case forfeit:
                - if <player.has_flag[Duel]>:
                    ...
                    - narrate "<player.name.display><&7> has forfeit the fight." target:<[user]> format:DuelFormat
...
DuelHeartbeat:
    ...
    script:
        ...
        - while !<server.flag[Duel_<[uuid]>_Over]>:
            ...
            - while <[user].is_npc>:
                - narrate "It's <&b><[user].name><&7>'s turn! (HP:<[user].flag[Duel_Health]>)" targets:<server.flag[Duel_<[uuid]>]> format:DuelFormat
                - random:
            ...
            - narrate "It's <&b><[user].name.display><&7>'s turn! (HP:<[user].flag[Duel_Health]>)" targets:<server.flag[Duel_<[uuid]>]> format:DuelFormat
...
DuelAttack:
    ...
    script:
        ...
        # Deal the damage
        - random:
            - narrate "<&b><[name]><&f><&7> attacks <&c><[character]><&7>! They rolled a <&e><[result]><&f>!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
            - narrate "<&b><[name]><&f><&7> goes to strike <&c><[character]><&7>! They rolled a <&e><[result]><&f>!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
            - narrate "<&b><[name]><&f><&7> lashes out at <&c><[character]><&7>! They rolled a <&e><[result]><&f>!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
            - narrate "<&b><[name]><&f><&7> focuses and darts at <&c><[character]><&7>! They rolled a <&e><[result]><&f>!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
        
        # If we get over a ten, we succeed.
        - if <[result]> >= 10:
            # If we get less than 15, roll 1d6.
            - if <[result]> < 15:
                - define damage:<util.random.int[1].to[6]>
            # Otherwise, roll 1d8
            - else:
                - define damage:<util.random.int[1].to[8]>
            # If  nat20, add a 1d5 to your attack too
            - if <[result]> == 20:
                - define damage:<[damage].add_int[<util.random.int[1].to[5]>]>
            # Check for preparation bonus
            - if <[user].has_flag[duel_power]> && <[user].flag[duel_power]> != 0:
                - define damage:+:<[user].flag[duel_power]>
                - narrate "<[name]> unleashed some power!" format:DuelActionFormat target:<server.flag[Duel_<[uuid]>]>
            ...
            # Deal the damage
            - else:
                - narrate "They dealt <[damage]> damage!" format:DuelActionFormat target:<server.flag[Duel_<[uuid]>]>
                ...
                - if <[target].flag[Duel_Health]> <= 0:
                    ...
                    - narrate "<[name]><&7> has fainted!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
                    - flag <[target]> Duel_KO
...
DuelDefend:
    ...
    script:
        ...
        - narrate "<[name]><&7> goes to brace themselves!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
    
        - define result:<util.random.int[1].to[20]>
        
        - if <[result]> >= 10:
            - if <[result]> < 15:
                - define protection:<util.random.int[1].to[6]>
                - narrate "<[name]><&7> rolled a <[result]> and gained <[protection]> points of protection!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
            - else:
                - define protection:<util.random.int[1].to[8]>
                - narrate "<[name]><&7> rolled a <[result]> and gained <[protection]> points of protection!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
            - if <[result]> == 20:
                - narrate "<[name]><&7> rolled a <[result]>! They have a full defense!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
                - define protection:20
            - flag <[user]> Duel_Protection:<[protection]>
        - else:
            - narrate "But they were too slow!" format:DuelActionFormat target:<server.flag[Duel_<[uuid]>]>
        - flag server Duel_<[uuid]>_waiting:!
...
DuelPrepare:
    ...
    script:
        ...
        - else:
            - define name:<[user].name.display>
        - narrate "<[name]><&7> stores energy!" format:DuelFormat target:<server.flag[Duel_<[uuid]>]>
        - define result:<util.random.int[<1>].to[20]>
        - if <[result]> >= 10:
            - if <[result]> < 15:
                - define bonus:<util.random.int[1].to[3]>
                - narrate "<[name]><&7> rolled a <[result]> and gained some power!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
            - else:
                - if <[result]> == 20:
                    - narrate "<[name]><&7> rolled a <[result] >and gained a lot of power!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
                    - define bonus:5
                - else:
                    - define bonus:<util.random.int[1].to[5]>
                    - narrate "<[name]><&7> rolled a <[result]> and gained a good bit of power!" target:<server.flag[Duel_<[uuid]>]> format:DuelActionFormat
            - if <[user].has_flag[Duel_Power]>:
                - flag <[user]> Duel_Power:+:<[bonus]>
            - else:
                - flag <[user]> Duel_Power:<[bonus]>
        - else:
            - narrate "But they failed!" format:DuelActionFormat target:<server.flag[Duel_<[uuid]>]>
        - flag server Duel_<[uuid]>_waiting:!
